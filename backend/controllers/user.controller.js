...
export const getUserProfile = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal in getUserProfile controller server error!" });
  }
};
...
export const followUnfollowUser = async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Internal in followUnfollowUser controller server error!",
    });
  }
};
...
export const getSuggestedUsers = async (req, res) => {
  try {
    ...
    const users = await User.aggregate([
      {
        $match: {
          _id: { $ne: userId }, // o'zimni chiqarib tashlash
        },
      },
      {
        $sample: {
          size: 10, // tasodifiy 10 ta user olish
        },
      },
    ]);
    ...
    res.status(500).json({
      error: "Internal in getSuggestedUsers controller server error!",
    });
  }
};
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: "Internal in updateUserProfile controller server error!",
    });
  }
};
